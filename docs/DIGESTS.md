...
- Validation: 36 model/hook scenarios, typecheck, real server/native item-payload contract checks, clean prebuild on iOS/Android and both Metro/Hermes exports passed. A signed iOS development build was installed and loaded on the registered physical device. Device registration and an Expo/APNs `ok` receipt passed; exact-item taps, visual/accessibility acceptance and Android delivery remain pending.
