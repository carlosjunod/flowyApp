...
## Monthly cadence — 2026-09-12

This is a new report cadence, separate from the existing monthly usage allowance. Deploy the matching Flowy monthly migration, API and worker from `codex/owner-usage-reading` first; see server `docs/owner-usage-reading.md` and `docs/digest-rollout.md` for the coordinated rollout. No digest was generated or delivered by this implementation task.

Settings now accept `monthly_enabled`, `monthly_day` (1–28), `monthly_local_time` (`HH:MM`), `monthly_push_enabled` and `monthly_email_enabled`. Backend defaults are disabled, day 1, 08:00, push off and email off. The native fields are optional to tolerate older servers: the monthly schedule card only appears once the server supplies `monthly_enabled`. Turning it on is an explicit unsaved edit until **Save choices** succeeds. Push and email remain independent opt-ins. Monthly and daily share the server's existing paid/Beta gate `canEnableDaily`; Free retains weekly. This does not expand any plan's monthly allowance or permit a native flag to authorize generation.

The publication date offers named accessible choices for days 1 through 28, which exist in every month, plus the existing native time picker and shared IANA timezone. Monthly content covers the previous complete calendar month in that timezone. On a coinciding publication day, monthly takes precedence over weekly and daily; all still share the same report/month and report/day limits. The server remains authoritative for schedule resolution, day validation, quotas, DST behavior and next publication.

Monthly-only schedules support pause/resume and **Turn off all digests**. Changing timezone or other preferences retains monthly day, local time and channel choices. The history cadence cycle now includes Monthly and preserves server cursor/read filters. The existing detail UI uses the report's cadence field, so monthly labels and content render without a separate screen.

Native files: `app/(app)/digest-settings.tsx`, `app/(app)/(tabs)/digest/index.tsx`, `src/lib/digestSettings.ts`, `src/lib/api.ts`, `src/types/index.ts`, `scripts/test-digest-monthly.cjs`, `package.json` and this handover. No new package, native capability or environment variable was introduced.

Validation passed: `npm run typecheck`, `npm run test:digest-monthly` (four model/real-screen/API groups), existing `npm run test:item-engagement` (four groups), and iOS Metro/Hermes export (1,953 modules) at `/private/tmp/flowy-native-monthly-export`. The monthly tests exercise opt-in defaults, 1–28 selection, settings persistence with revision/timezone/exclusions, independent channel consent, paid versus Free gating, older-server behavior, monthly-only pause/off, dirty detection and authenticated history filters. They use synthetic adapters and never register a device, generate a report or send push/email. The previously documented full UI-model runner issue and unverified physical-device accessibility remain limitations; Metro export is not device E2E.

Before release, confirm on authorized staging that native/web edits reflect each other; Free cannot enable monthly; paid/Beta can enable/save/disable it; month-boundary and DST schedules use the previous calendar month; an overlapping schedule publishes at most once; all cadence usage consumes the unchanged allowance. Inspect the 28-day selector with VoiceOver/TalkBack and large text, then follow the existing separately authorized delivery acceptance procedure above.

## Editorial editions — 2026-09-16 (branch `codex/digest-editorial`)

Server counterpart: Flowy `codex/digest-editorial` (D-039, `docs/digest-editorial.md`). Not deployed.

- Reports may carry an optional `content.editorial` edition: a cover and, for weekly/monthly, up to two interior images bound to body blocks. List and detail responses add a top-level `presentation: {editorial, pdf}`. Older servers omit both; this client then renders as before. Older clients ignore the new fields.
- `src/lib/digestEditorial.ts` validates the edition structurally (no zod): version 1, illustrated mode, approved image id and variant key shapes, https URLs (loopback only in `__DEV__` previews), focal range, alt text, no duplicate images. Anything invalid renders typographically. It mirrors the server labels and `periodLabel`, which treats `window_end` as exclusive (the previous header showed one extra day).
- `src/components/digest/EditorialImage.tsx`: `expo-image` in a stable 3:2 frame with focal `contentPosition`, the smallest JPEG variant covering layout width × pixel ratio (never upscaled), localized alt text as accessibility label, removal on load failure, discreet “AI-generated editorial illustration” caption, no tint or filter in either theme.
