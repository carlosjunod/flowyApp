...
- `src/hooks/usePushRegistration.ts`: existing-permission registration on login/foreground/token rotation; permission prompt only after explicit settings action; Android creates the high-importance `updates` channel first and registration returns a typed, user-visible result. Uses the existing EAS project ID. Replaces the obsolete `Constants.isDevice` gate.
...
## Mandatory device run — in progress

Use a physical iPhone/Android device and an explicitly authorized own test account. Expo Go/simulator or an accepted Expo ticket does not satisfy this gate. Keep server staging user allowlist active. EAS supplies the matching Firebase file and FCM V1 credential. On September 11, iOS development build `2fbc5968-d25b-4d3e-b4de-813e911fad12` was re-signed as `b1c7a091-070b-4ab2-9c4f-db6cb8c68fbf`, installed on an owned iPhone and loaded through Metro. Registration initially stalled because the React Native runtime lacked the static timeout helper and concurrent foreground/settings events queued repeated attempts. The client now uses `AbortController`, shares one active registration and always releases the settings spinner. Production stored the refreshed Expo token and revocation hash; an owned-device test ticket received an `ok` Expo/APNs receipt and visibly arrived on the physical lock screen. Android development build `a2ee45bd-d9da-4337-bf24-cdfd2fb581de` was then installed on an owned phone and loaded through Metro. Its registration, `ok` Expo/FCM receipt, visible high-priority test notification and a worker-generated `Ready to read` notification all passed after creating the `updates` channel. The native bundle remains `app.tryflowy.client` (web Apple Services ID remains `app.tryflowy.app`). Complete the tap scenarios below.
...
- Validation: 36 model/hook scenarios, typecheck, real server/native item-payload contract checks, clean prebuild on iOS/Android and both Metro/Hermes exports passed. Signed iOS and Android development builds were installed on registered physical devices. Registration, provider `ok` receipts and visible arrival passed on both; Android also displayed a real worker-generated item notification. Exact-item/report taps and broader visual/accessibility acceptance remain pending.
