...
Use a physical iPhone/Android device and an explicitly authorized own test account. Expo Go/simulator or an accepted Expo ticket does not satisfy this gate. Keep server staging user allowlist active. The September 10 push follow-up configures a default Android channel; EAS now supplies the matching Firebase file and FCM V1 credential. On September 11, iOS development build `2fbc5968-d25b-4d3e-b4de-813e911fad12` was re-signed as `b1c7a091-070b-4ab2-9c4f-db6cb8c68fbf` with refreshed app/share-extension profiles, installed on an owned iPhone and loaded through Metro. The native bundle remains `app.tryflowy.client` (web Apple Services ID remains `app.tryflowy.app`). Complete the delivery and tap scenarios below.
...
- Validation: 35 model/hook scenarios, typecheck, real server/native item-payload contract checks, clean prebuild on iOS/Android and both Metro/Hermes exports passed. A signed iOS development build was installed and loaded on the registered physical device. Physical notification receipt/taps and visual/accessibility acceptance are pending.
