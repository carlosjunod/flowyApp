...
Android builds receive `GOOGLE_SERVICES_JSON` from EAS for development, preview and production. Firebase project `flowy-494202` contains the matching `app.tryflowy.client` app, and Expo stores its FCM V1 service-account credential server-side; neither file is committed or embedded in the app. The client creates the `updates` channel at high importance before permission/registration, and push payloads select that channel explicitly. The plugin retains `default` as the native fallback. Firebase configuration changes require a fresh native build.

Run `npm run typecheck`, `npm run test:ui-models`, and `npm run test:push-contract -- /absolute/path/to/Flowy` against the paired server checkout. Clean prebuild and iOS/Android Metro/Hermes exports passed in isolated worktrees. The signed iOS development build completed registration, returned an `ok` Expo/APNs receipt and displayed on the lock screen. Android development build `a2ee45bd-d9da-4337-bf24-cdfd2fb581de` installed on an owned phone; registration, an `ok` Expo/FCM receipt, visible high-priority test delivery and a real worker-generated `Ready to read` notification passed. Exact-item tap scenarios remain release checks; see `docs/DIGESTS.md` and server `docs/push-notifications-rollout.md` before broad production report delivery.
