...
Android builds receive `GOOGLE_SERVICES_JSON` from EAS for development, preview and production. Firebase project `flowy-494202` contains the matching `app.tryflowy.client` app, and Expo stores its FCM V1 service-account credential server-side; neither file is committed or embedded in the app. The `expo-notifications` plugin uses the `default` channel. This configuration change requires a fresh native build.

Run `npm run typecheck`, `npm run test:ui-models`, and `npm run test:push-contract -- /absolute/path/to/Flowy` against the paired server checkout. Clean prebuild and iOS/Android Metro/Hermes exports passed in isolated worktrees. An iOS development build was signed, re-provisioned for the connected owned iPhone and installed successfully. On-device registration then completed; an Expo/APNs test returned an `ok` delivery receipt and appeared on the physical lock screen. Exact-item tap scenarios and Android delivery remain release checks; see `docs/DIGESTS.md` and server `docs/push-notifications-rollout.md` before broad production report delivery.
