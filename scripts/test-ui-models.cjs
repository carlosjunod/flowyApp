...
(async () => {
  ...
  const options = items.useItems({ userId: 'account-a', sortField: 'created', sortDir: 'desc', search: ' older ', category: ' Tech ', tag: ' Design ' });
  const page = await options.queryFn({ pageParam: 3 });
  assert.deepEqual(searchParams, { tag: 'design', unread: undefined, page: 3, perPage: 20, q: 'older', category: 'technology', sort: 'date', direction: 'desc' });
