...
(async () => {
  ...
  fail=true;state=render();await state.toggleRead();state=render();assert.equal(state.errorKey,'inbox.read.syncFailed');fail=false;
  ...
  assert.equal(invalidations.length,oldInvalidations);assert.equal(render().errorKey,null);assert.ok(unsubscribed>0);
  harness.close();
