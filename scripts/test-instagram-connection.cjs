...
(async () => {
  ...
  createResult=async()=>({data:connection,error:null});
  accountId='alice';destination=undefined;authStore.model={id:'alice'};authStore.token='token-a';
  queryData={enabled:true,referralEnabled:true,connected:false};
  state=render();
  assert.equal(state.handoffReturned,false);
  await state.connect();state=render();
  assert.equal(opened.at(-1),'https://ig.me/m/tryflowy?ref='+code);
  assert.equal(state.handoffReturned,false);
  appState.currentState='active';foreground('active');state=render();
  assert.equal(state.handoffReturned,true);
  queryData={...queryData,connected:true};state=render();
  assert.equal(state.handoffReturned,false);
  assert.equal(state.connection,null);
  passed('handoff returns unconnected: foreground with an unresolved handoff surfaces the hint until status confirms connection, which clears the issued code');
  ...
  appState.currentState='active';foreground('active');state=render();
  assert.equal(state.handoffReturned,true);
  await state.connect(true);state=render();
  assert.equal(state.handoffReturned,false);
  passed('regenerate clears handoff: connect(true) resets handoffReturned to false');

  await state.disconnect();state=render();
  const connectionWithUsername={...connection,username:'some.user'};
  createResult=async()=>({data:connectionWithUsername,error:null});
  queryData={enabled:true,referralEnabled:true,connected:false};
  state=render();
  assert.equal(state.connection,null);
  await state.connect();state=render();
  assert.equal(state.data.username,'some.user');
  assert.equal(state.connection.username,'some.user');
  assert.equal(cached.at(-1).username,'some.user');
  assert.equal('code' in cached.at(-1),false);
  assert.equal('expiresAt' in cached.at(-1),false);
  passed('username passes through: the linked username is available on data (and each cached status) while code/expiresAt stay out of the query cache');
