...
export function InstagramConnectionScreen({ accountId }: { accountId?: string }) {
  ...
  const state = useInstagramConnection(accountId);
    ...
    <SafeAreaView className="flex-1 bg-bg" edges={['top', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40, gap: 20 }}>
        <View className="gap-2">
          ...
          <Text className="text-muted text-sm leading-5">Connect your Instagram account, then send public Instagram or TikTok post links to @tryflowy. Flowy saves and processes them with AI in your library.</Text>
          <Text className="text-muted text-xs leading-5">For shared posts or reels, you may need to copy and send the public link.</Text>
        </View>
        ...
        {state.data?.connected ? <View className="rounded-xl border border-border bg-card p-4 gap-4">
          ...
          <Text className="text-muted text-sm leading-5">Send your next post link to @tryflowy.</Text>
          ...
          <Text className="text-fg text-sm leading-5">{state.data.referralEnabled
            ...
            : 'Generate a private code and send it to @tryflowy from the Instagram account you want to connect.'}</Text>
          ...
          {showCode && <>
            ...
            {(manual || !state.data.referralEnabled) && <View className="gap-3">
              <Text className="text-muted text-sm">Send this code to @tryflowy as a message.</Text>
