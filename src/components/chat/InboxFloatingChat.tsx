...
export function InboxFloatingChat({ open, onOpenChange, onShowSources }: {
  ...
  if (!open) return <Pressable ref={launcher} onPress={() => onOpenChange(true)} accessibilityRole="button" accessibilityLabel={chat.unread ? t('chat.inboxChat.askReady') : t('chat.inboxChat.ask')} accessibilityState={{ expanded: false }}
  ...
  return <View pointerEvents="box-none" style={{ position: 'absolute', top: 8, right: 8, bottom: 8, left: 8, alignItems: 'flex-end', justifyContent: 'flex-end' }}>
    <View accessibilityViewIsModal accessibilityLabel={t('chat.inboxChat.panelLabel')} onAccessibilityEscape={close}
      ...
      <ChatPanel compact focused onShowSources={ids => { close(); onShowSources(ids); }} actions={<>
        <Pressable accessibilityRole="button" accessibilityLabel={t('chat.inboxChat.openFull')} style={action} onPress={() => { close(); router.navigate('/chat'); }}><Feather name="arrow-up-right" color={colors.fg} size={20} /></Pressable>
        <Pressable ref={closeButton} accessibilityRole="button" accessibilityLabel={t('chat.inboxChat.minimize')} style={action} onPress={close}><Feather name="minus" color={colors.fg} size={20} /></Pressable>
      </>} />
    </View>
  </View>;
}
