...
/** Swipe right to return. iOS delegates to its interactive native-stack gesture. */
export function ReaderSwipe({ children, onClose, scrollGesture, disabled = false }: {
  ...
  const gesture = useMemo(() => Gesture.Pan()
    .enabled(!disabled && Platform.OS !== 'ios')
    // The scroll view waits only inside this edge until direction is known.
    ...
    .failOffsetY([-12, 12])
    .maxPointers(1)
    .onUpdate(event => { if (!closing.value) x.value = Math.max(0, event.translationX); })
    .onEnd(event => {
