...
function External({ url, label, compact = false }: { url?: string; label: string; compact?: boolean }) {
  ...
  </Pressable>{failed ? <Text accessibilityRole="alert" className="text-muted text-sm">Could not open this link. Try again.</Text> : null}</View>;
}
  ...
  return <View style={{ gap: 12 }}>
    {author ? <View>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        ...
        <External url={author.url} label={`Open @${author.handle} on Instagram`} compact />
      </View>
      {item.author_key === `instagram:${author.handle}` ? <Pressable accessibilityRole="button" className="active:opacity-70" style={{ minHeight: 44, paddingLeft: 56, paddingRight: 12, paddingVertical: 8, marginTop: 4, flexDirection: 'row', gap: 12, alignItems: 'center' }}
        ...
        <Text style={{ flex: 1, fontFamily: 'Inter_500Medium', fontSize: 14, lineHeight: 20, color: colors.accent }}>{READER_COPY.authorSaves}</Text><Feather name="arrow-right" size={17} color={colors.accent} />
      </Pressable> : null}
    </View> : null}
    {repository ? <View className="rounded-xl border border-border p-4" style={{ gap: 4 }}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 20 }}><External url={githubRepository(repository.url)?.url} label="Open repository" /><External url={item.source_metadata?.repository?.homepage} label="Project website" /></View>
