...
import { INSTAGRAM_CHAT_URL, instagramConnectionError, instagramConnectionUrl } from '@/lib/instagramConnection';
import { pb } from '@/lib/pb';
import type { ApiError, ApiResult, InstagramConnection } from '@/types';

export const instagramQueryKey = (accountId?: string) => ['instagram-connection', accountId] as const;

export function useInstagramConnection(accountId?: string) {
  const client = useQueryClient();
  const active = useRef({ accountId, mounted: true, busy: false });
  const [issued, setIssued] = useState<{ accountId: string; connection: InstagramConnection } | null>(null);
  ...
  const query = useQuery<InstagramConnection, ApiError>({
    queryKey: instagramQueryKey(accountId), enabled: !!accountId, staleTime: 0, retry: false,
    queryFn: async ({ signal }) => {
      const result = await api.getInstagramConnection(accountId!, signal);
      if (result.error) throw result.error;
      return result.data;
    },
  });
  const connection = issued?.accountId === accountId && !query.data?.connected ? issued?.connection : null;
  ...
  function current(scope: typeof active.current) {
    return !!accountId && scope.mounted && scope === active.current && scope.accountId === accountId && pb.authStore.model?.id === accountId;
  }
  async function open(url: string) {
    ...
    catch { if (current(scope)) setError('Could not open Instagram. Open @tryflowy in the Instagram app and send the connection code below.'); }
  }
  async function mutate(operation: () => Promise<ApiResult<InstagramConnection>>) {
    ...
    try {
      await client.cancelQueries({ queryKey: instagramQueryKey(accountId) });
      ...
      client.setQueryData(instagramQueryKey(accountId), status);
      setIssued(code ? { accountId: accountId!, connection: result.data } : null);
      setNow(Date.now());
      return result.data;
    } catch {
  ...
  async function connect(regenerate = false) {
    ...
    const next = await mutate(() => api.createInstagramConnection(accountId!));
  ...
  return {
    ...
    openChat: () => open(INSTAGRAM_CHAT_URL),
    disconnect: () => mutate(() => api.disconnectInstagram(accountId!)),
  };
}
